...
class DataTrainingArguments:
    ...
    eval_every: Optional[int] = field(
        default=None,
        metadata={
            "help": ("Number of steps between evaluations (MUST be set if patience is set).")
        },
    )
...
class ModelArguments:
    ...
    freeze_model: bool = field(
        default=False, metadata={"help": "Whether to freeze the parameters of the base model."}
    )
    ...
    trust_remote_code: bool = field(
        default=False,
        metadata={"help": "Trust remote code (e.g. from huggingfacehub) when loading model."},
    )
...
def main():
    ...
    if data_args.patience:
        ...
        callbacks = [
            EarlyStoppingCallback(
                early_stopping_patience=data_args.patience, early_stopping_threshold=0.001
            )
        ]
    else:
        callbacks = None
    ...
    if data_args.task_name is not None:
        ...
        non_label_column_names = [
            name for name in raw_datasets["train"].column_names if name not in ("label", "idx")
        ]
    ...
    def preprocess_function(examples):
        ...
        tokenizer_kwargs = {"padding": padding, "max_length": max_seq_length, "truncation": True}
        if "postags" in model_args.model_name_or_path:
            tokenizer_kwargs["return_pos_tag_ids"] = True
        result = tokenizer(*args, **tokenizer_kwargs)
